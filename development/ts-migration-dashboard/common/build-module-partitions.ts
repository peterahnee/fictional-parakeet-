...
function buildModulesWithLevels(
  ...
  // Question: Why is shared/constants/network.ts,
  // shared/constants/transaction.ts, and shared/modules/hexstring-utils.ts not
  // in level 38?
  ...
  while (modulesToFill.length > 0) {
    ...
    for (const nonCanonicalDependencyModuleId of dependenciesByModuleId[
      ...
      const existingDependencyModule = modulesById[dependencyModuleId];
      ...
      if (existingDependencyModule === undefined) {
        ...
        dependencyModulesToFill.push(dependencyModule);
      } else {
        if (currentModule.level + 1 > existingDependencyModule.level) {
        ...
        if (
          !existingDependencyModule.dependents.some(
            (dependent) => dependent.id === currentModule.id,
          )
        ) {
        ...
        if (
          !currentModule.dependencies.some(
            (dependency) => dependency.id === existingDependencyModule.id,
          )
        ) {
