...
If it's repeated through a test suite, it can be put at the top so that it can be used by multiple test blocks.
...
describe('<GasEstimator />', () => {
  ...
  it('renders likelihood indicator', async () => {
    await waitFor(() => {
  ...
  it('renders user address', async () => {
    await waitFor(() => {
      expect(queryByText(/Address:/u)).toBeInTheDocument();
    });
  });
});
```

State or props data will often be much more complex and verbose than in the example above, and we'll want to do slight modifications between tests. To avoid duplication hell and enhance readability, it's sometimes helpful to transform the base objects into object generators that accept overrides.
